function parameters = ExternalFunctionTest_parameters()
    ...
    %% Options and problem
      parameters.Name = 'ExtFunTest'; % MOD
      parameters.verbose = 5;
      parameters.save_data = 0;
      % Problem
      parameters.problem = 'ExternalFunctionTest'; % MOD - /!\ Name of the problem. This name relates to the file "toy_problem.m" in the plant/toy_problem folder.
      parameters.problem_type = 'external'; % "internal" for evaluation of the individuals in the same matlab session; 'external' for experiment or coupling with a solver.
      parameters.interpolation_type = 'internal'; % If "external" compute the secondary MLC problem outside the original session.
      parameters.external_interpolation = 0; % Should not be used.

parameters.ExternalStop=125;
parameters.ExternalStopExi=-1;
%         ProblemParameters.fortran_evaluation = 0; % Not coded yet
...
%% gMLC parameters
  ...
  %% Problem parameters
        ...
        ProblemParameters.PathExt = 'OUTPUT/Costs/'; % useless if "internal" type of problem. Choose your own external path
        ProblemParameters.MLCOutputPath = []; % Path to send the Sets for the evaluation. To set if this necessary.
        ProblemParameters.fortran_evaluation = 0; % Not coded yet
    % Definition
